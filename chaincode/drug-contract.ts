...
export class DrugContract extends Contract {
	private getCollectionName(ctx: Context, pharmacyMsp?: string): string {
		...
		throw new Error(
			`MSP ID ${mspId} nemá priradenú žiadnu súkromnú kolekciu.`,
		);
	}
	...
	@Transaction()
	public async addDrugDefinition(
		ctx: Context,
		id: string,
		name: string,
		composition: string,
	...
	@Transaction()
	public async providePriceOffer(
		ctx: Context,
		requestID: string,
		pharmacyMsp: string,
	): Promise<void> {
	...
	@Transaction()
	public async finalizeAgreement(
		ctx: Context,
		requestID: string,
	): Promise<void> {
		const pharmacyMsp = ctx.clientIdentity.getMSPID();
	...
	public async transferOwnership(
		...
		if (requestID) {
			const orderCol = this.getCollectionName(
				ctx,
				newOwnerOrg.startsWith("Lekaren") ? newOwnerOrg : mspId,
			);
			const orderJSON = await ctx.stub.getPrivateData(
				orderCol,
				requestID,
			);
			if (orderJSON && orderJSON.length > 0) {
	...
	public async sellToConsumer(ctx: Context, batchID: string): Promise<void> {
		...
		if (qSold > priv.quantity)
			throw new Error(
				`Nedostatok zásob. Požadované: ${qSold}, Dostupné: ${priv.quantity}`,
			);
	...
	@Transaction()
	public async returnToManufacturer(
		ctx: Context,
		batchID: string,
		manufacturerOrg: string,
	): Promise<void> {
	...
	public async emergencyRecall(ctx: Context, batchID: string): Promise<void> {
		if (ctx.clientIdentity.getMSPID() !== "SUKLMSP") {
			throw new Error(
				"Iba regulátor (ŠÚKL) môže vyhlásiť sťahovanie z trhu.",
			);
		}
	...
	public async queryHistory(ctx: Context, batchID: string): Promise<string> {
		...
		allResults.sort((a, b) => {
			const timeA =
				Number(a.timestamp.seconds) + Number(a.timestamp.nanos) / 1e9;
			const timeB =
				Number(b.timestamp.seconds) + Number(b.timestamp.nanos) / 1e9;
			return timeB - timeA;
		});
	...
	@Transaction(false)
	public async readDrugDefinition(
		ctx: Context,
		id: string,
	): Promise<Uint8Array> {
	...
	@Transaction(false)
	@Returns("string")
	public async queryPrivateOrders(
		ctx: Context,
		pharmacyMsp: string,
	): Promise<string> {
		const collection = this.getCollectionName(ctx, pharmacyMsp);
		const registryBytes = await ctx.stub.getPrivateData(
			collection,
			"REGISTRY_ORDERS",
		);
		if (!registryBytes || registryBytes.length === 0)
	...
	@Transaction()
	public async RegisterExistingKey(
		ctx: Context,
		registryKey: string,
		id: string,
		collection?: string,
